...
uint8_t can_init(uint32_t can_br, uint8_t num_tx_mb, uint8_t num_rx_mb) {
    ...
    // Disable the Parallel IO (PIO) of the Rx and Tx pins so that the
...
uint8_t can_send(CAN_MESSAGE *can_msg, uint8_t tx_mb_id) {
    ...
    if (CAN0->CAN_MB[tx_mb_id].CAN_MSR & CAN_MSR_MRDY) {
        ...
        CAN0->CAN_MB[tx_mb_id].CAN_MID =
            CAN_MID_MIDvA(can_msg->id) | CAN_MID_MIDE;
        ...
        CAN0->CAN_MB[tx_mb_id].CAN_MDL =
            can_msg->data[3] << 24 | can_msg->data[2] << 16 |
            can_msg->data[1] << 8 | can_msg->data[0];
        CAN0->CAN_MB[tx_mb_id].CAN_MDH =
            can_msg->data[7] << 24 | can_msg->data[6] << 16 |
